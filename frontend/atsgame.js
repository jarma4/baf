...
function getAtsPicks(season, week) {
	...
	if (week == getWeek(new Date(), 'nfl') && ((day == 3 || (day == 4 && hour < 19)))){
		$('.atsInc').removeClass('hidden');
      document.getElementById("atsPicks").innerHTML = '<h4 class="center">Come back Fri</h4>';
   	...
   	.then(retData => {
         ...
         retData.ats.forEach((rec, i) => {
				if (retData.picks.length > 1) {  // showing everyone's picks
					...
					outp += '<tr><td class="td-odds"><button class="btn '+((retData.picks[0][i] == '1')?'btn-success':'btn-primary')+((new Date(rec.date) > new Date())?' btn-toggle':' disabled')+'" data-game="'+i+'" data-team="1"><table class="btnIcon"><tr><td rowspan="2" width="20px"><img id="tm1_'+i+'" class="logo-md" src="images/nfl_logo_sprite_medium.png?ver=1"></td><td class="center">'+rec.team1.slice(0,5)+'</td></tr><tr><td class="center bold">'+rec.spread+'</td></tr></table></button></td>';
					outp += '<td class="td-odds"><button class="btn '+((retData.picks[0][i] == '2')?'btn-success':'btn-primary')+((new Date(rec.date) > new Date())?' btn-toggle':' disabled')+'" data-game="'+i+'" data-team="2"><table class="btnIcon"><tr><td rowspan="2" width="20px"><img id="tm2_'+i+'" class="logo-md" src="images/nfl_logo_sprite_medium.png?ver=1"></td><td class="center">'+rec.team2.slice(0,5)+'</td></tr><tr><td class="center bold">'+(0-rec.spread)+'</td></tr></table></button></td></td></tr>';
				}
			});
         outp += '</table>';
